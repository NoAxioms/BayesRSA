...
def model(target_item = 0, num_utterances_heard = 5, utterances_heard = None, use_rsa=False, verbose = False):
	#Make sure each item has at least one word. Options: add "thing" as dummy word that is always valid. Sample s_0 from dirichlet.
	num_items = 3
	vocab_size = 3
	...
	#Replace loop with single sample
	for item_num in item_plate:
		item_vocabs[item_num][-1] = 1
		for word_num in vocab_plate:
			item_vocabs[item_num][word_num] = pyro.sample("item_vocabs_{}_{}".format(item_num,word_num), dist.Bernoulli(0.5))
	if verbose: print("item_vocabs:\n{}".format(item_vocabs))
	s_2 = rsa(s_0 = item_vocabs) if use_rsa else normalize(item_vocabs)  #[s][u]
	...
	if verbose: print("utterances_heard:\n",utterances_heard)
	return s_2
		
def guide(target_item = 0, num_utterances_heard = 5, utterances_heard = None, use_rsa=False, verbose = False):
	num_items = 3
	vocab_size = 3
	#I probably need to convert these to sequential plates since they couple downstream
	item_plate = pyro.plate('item_plate', num_items)
	...
	if verbose: print("item_vocabs:\n{}".format(item_vocabs))
	s_2 = rsa(s_0 = item_vocabs) if use_rsa else normalize(item_vocabs)  #[s][u]
	if verbose: print("s_2: {}".format(s_2))
	#Should use histogram instead of sequence of utterances
	utterance_plate = pyro.plate('utterance_plate', num_utterances_heard, dim=-1)
	with utterance_plate as u_id:
		#Must I remove the obs?
		utterances_heard = pyro.sample('utterances_heard',dist.Categorical(s_2[target_item]), obs=utterances_heard) #[u_id]
	if verbose: print("utterances_heard:\n",utterances_heard)
	return s_2
